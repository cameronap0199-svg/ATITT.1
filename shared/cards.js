...
const laneX = (x) => Math.floor(x / C.LANE_WIDTH);
const laneU = (u) => laneX(u.x);
...
// Starting foothold (not collectible)
REGISTRY.set('TK-homeland', { id: 'TK-homeland', token: true, type: 'Zone', name: 'Homeland', cls: 'Territory', faction: 'Character', archetype: 'Ancestry', rarity: 'Base', cost: 0, emoji: '🏕️', text: 'Your starting Lane. It has no special effect — but it is home.', hooks: null });
REGISTRY.set('TK-basecamp', { id: 'TK-basecamp', token: true, type: 'Structure', name: 'Base Camp', cls: 'Shelter', faction: 'Character', archetype: 'Infrastructure', rarity: 'Base', cost: 0, bp: 12, housing: 3, emoji: '⛺', text: 'Your starting Structure. Identities can be summoned next to it.', hooks: null });

token('TK-spiderling', 'Spiderling', 'Arthropod', '🕷️', { bp: 1, sp: 2, mp: 1, ap: 4, rp: 1 }, 'Tiny, fast, and there are always more.');
token('TK-ant', 'Worker Ant', 'Arthropod', '🐜', { bp: 2, sp: 2, mp: 1, ap: 3, rp: 1 }, 'For the Queen!');
token('TK-drone', 'Knot-Bot Drone', 'Knot-Bots', '🛸', { bp: 3, sp: 2, mp: 1, ap: 3, rp: 2 }, 'Beep boop. Hostile detected.');
token('TK-wisp', 'Wisp', 'Ghoul', '👻', { bp: 1, sp: 1, mp: 1, ap: 4, rp: 2 }, 'A candle flame with a grudge.');
token('TK-minnow', 'Minnow', 'Scales', '🐟', { bp: 1, sp: 1, mp: 1, ap: 4, rp: 1 }, 'Just keep swimming.');
token('TK-hatchling', 'Hatchling', 'Feathered', '🐣', { bp: 2, sp: 1, mp: 1, ap: 3, rp: 2 }, 'Peep!');
...
const isEnemyUnit = (E, m, id) => { const u = E.unit(id); return u && u.owner !== m.owner ? u : null; };
...
shared('Silviculturist', 'Stagnation', 'Pressure', 'Enemy Identities adjacent to this Identity while it occupies a Lane you control have -1 AP. This effect does not stack.', {
  stat(E, src, u, k) {
    if (k !== 'ap' || u.owner === src.owner) return 0;
    const m = me(E, src);
...
shared('Silviculturist', 'Monopoly', 'Community', "When all friendly Identities occupying a Lane share this Identity's Faction, the first damage dealt to a friendly Structure in this Lane each turn is reduced by 1.", {
  incoming(E, src, dmg) {
    ...
    if (!m || !st || st.owner !== m.owner || st.lane !== laneU(m) || !E.laneMonopoly(m)) return;
...
shared('Silviculturist', 'Predation', 'Territorial', 'This Identity gains +1 SP when attacking an enemy Identity occupying a Lane you control.', {
  attackBonus(E, src, u, t, ctx) { return u.iid === src.unit && !ctx.isStruct && E.laneCtrl(laneU(t)) === u.owner ? 1 : 0; },
});
...
shared('Hydrologist', 'Hardpan', 'Roll With It', 'Whenever an enemy effect would forcibly move this Identity, reduce that movement by 1 tile. After the movement resolves, this Identity may move 1 tile.', {
  forcedDelta(E, src, u, byP) { return u.iid === src.unit && byP !== u.owner ? -1 : 0; },
  on(E, src, ev, d) { if (ev === 'forcedMove' && d.unit === src.unit && d.by !== src.owner) freeStep(E, me(E, src), 'Roll With It'); },
});
...
shared('Hydrologist', 'Monopoly', 'Follow The Leader', "When all friendly Identities occupying a Lane share this Identity's Faction, the first friendly Identity sharing this Identity's Faction that moves in this Lane each turn may move 1 additional tile.", {
  on(E, src, ev, d) {
    ...
    const m = me(E, src); const o = E.unit(d.unit);
    if (!m || !o || o.owner !== m.owner || !E.sameFaction(o, m) || laneU(o) !== laneU(m) || !E.laneMonopoly(m)) return;
...
shared('Understory', 'Hardpan', 'Steady', 'While this Identity is adjacent to another friendly Identity, the first enemy effect each turn that would forcibly move it moves it 1 fewer tile.', {
  forcedDelta(E, src, u, byP) {
    if (u.iid !== src.unit || byP === u.owner || !E.friendlyAdjacent(u).length) return 0;
...
shared('Understory', 'Scarcity', 'Swiftness', 'If friendly Identities are outnumbered in this Lane, this Identity gains +1 AP. If it ends movement adjacent to another friendly Identity, that Identity also gains +1 AP during its next activation.', {
  stat(E, src, u, k) {
    ...
    const l = laneU(u);
    return E.unitsInLane(l, u.owner).length < E.unitsInLane(l, 1 - u.owner).length ? 1 : 0;
  },
  on(E, src, ev, d) {
    if (ev !== 'moved' || d.unit !== src.unit || d.forced) return;
...
shared('Understory', 'Monopoly', 'Room For One More', "When all friendly Identities occupying a Lane share this Identity's Faction, the friendly Structure in this Lane gains +1 Housing. This effect does not stack.", {
  housing(E, src, st) {
    const m = me(E, src);
    return m && st.owner === m.owner && st.lane === laneU(m) && E.laneMonopoly(m) ? { v: 1, key: 'roomForOneMore' } : 0;
  },
});
...
shared('Poacher', 'Succulence', 'Still Standing', 'Whenever this Identity defeats an enemy Identity, restore 2 BP.', {
  on(E, src, ev, d) { if (ev === 'defeated' && d.killer === src.unit && d.owner !== src.owner) E.heal(me(E, src), 2); },
});
...
shared('Poacher', 'Hardpan', 'Agitation', 'Whenever an enemy effect forcibly moves this Identity, its next attack that turn gains +1 SP.', {
  on(E, src, ev, d) {
    if (ev === 'forcedMove' && d.unit === src.unit && d.by !== src.owner) E.addStatus(me(E, src), { kind: 'nextAttack', v: 1, at: 'eot', label: 'Agitated · +1 SP' });
  },
});
...
shared('Poacher', 'Monopoly', 'Home Team', "When all friendly Identities occupying a Lane share this Identity's Faction, this Identity gains +1 SP when attacking an enemy occupying this Lane.", {
  attackBonus(E, src, u, t, ctx) {
    ...
    const tl = t.lane !== undefined && t.x === undefined ? t.lane : laneU(t);
    return tl === laneU(u) ? 1 : 0;
  },
});
...
shared('Poacher', 'Infrastructure', 'First Impression', 'During the turn this Identity is summoned, its first attack against an enemy Identity or Structure occupying that Lane gains +1 SP.', {
  attackBonus(E, src, u, t) {
    if (u.iid !== src.unit || u.summonedSerial !== E.s.turnSerial || u.attacks !== 1) return 0;
    const tl = t.lane !== undefined && t.x === undefined ? t.lane : laneU(t);
...
shared('Forager', 'Ancestry', 'Peace of Mind', "Once per turn, when another friendly Identity sharing this Identity's Faction uses a Consumable in the same Lane, restore 1 MP to this Identity.", {
  on(E, src, ev, d) {
    ...
    const m = me(E, src); const o = E.unit(d.unit);
    if (m && o && o.owner === m.owner && E.sameFaction(o, m) && laneU(o) === laneU(m) && E.once(m, 'peaceOfMind')) E.restoreMp(m, 1);
  },
});
...
shared('Forager', 'Hardpan', 'Refuse', 'This Identity may spend 1 MP to prevent itself from being forcibly moved by an enemy effect.', {
  forcedImmune(E, src, u, byP) {
    if (u.iid !== src.unit || byP === u.owner || u.mp < 1) return false;
    E.loseMp(u, 1);
    return true;
  },
});
...
shared('Forager', 'Predation', 'Keep It Coming', 'Whenever this Identity defeats an enemy Identity, restore 1 MP. If it has an empty Consumable slot, you may also attach a Consumable from your hand to it.', {
  on(E, src, ev, d) {
    if (ev !== 'defeated' || d.killer !== src.unit || d.owner === src.owner) return;
    const m = me(E, src);
    E.restoreMp(m, 1);
    E.autoAttachCons(m);
  },
...
const T = {
  ...
  pushAll: (n, r, cost, name = 'Shockwave') => ({ name, text: `Push each enemy Identity within ${r} tile${r > 1 ? 's' : ''} ${n} tile${n > 1 ? 's' : ''} away.`, active: { name, cost, targets: [], condition: (E, c) => E.allUnits().some((u) => u.owner !== c.p && ch(u, E.unit(c.unit)) <= r), resolve: (E, c) => { const m = E.unit(c.unit); for (const u of E.allUnits().filter((x) => x.owner !== c.p && ch(x, m) <= r).sort((a, b) => ch(b, m) - ch(a, m))) E.push(u, m, n, c.p, { srcUnit: c.unit }); } } }),
  ...
  swapAny: (n, cost, name = 'Whirlpool Step') => ({ name, text: `Swap places with any other Identity within ${n} tiles.`, active: { name, cost, targets: [{ type: 'unit', side: 'any', range: n, notSelf: true, label: 'Swap with which Identity?' }], resolve: (E, c) => { const t = E.unit(c.targets[0]); if (t.owner !== c.p && E.hookAny('forcedImmune', t, c.p)) return; E.swap(E.unit(c.unit), t); } } }),
  ...
  sapTap: (n, cost, name = 'Transmute') => ({ name, text: `Gain ${n} Sap.`, active: { name, cost, oncePerTurn: true, targets: [], resolve: (E, c) => E.gainSap(c.p, n) } }),
  ...
  execute: (n, cost, name) => ({ name, text: `Deal ${n} damage to target enemy Identity within RP. If it is defeated, this Identity gains +2 AP and may attack again.`, active: { name, cost, targets: [enemyInRange()], resolve: (E, c) => { dmgA(E, c, c.targets[0], n); if (!E.unit(c.targets[0])) { const m = E.unit(c.unit); E.addApBonus(m, 2, name); m.extraAttacks++; } } } }),
  ...
  factionAura: (fac, name) => ({ name, text: `Other friendly ${fac} Identities within 2 tiles gain +1 SP.`, hooks: { stat(E, src, u, k) { if (k !== 'sp' || u.iid === src.unit || u.owner !== src.owner) return 0; const m = me(E, src); return m && E.faction(u) === fac && ch(u, m) <= 2 ? 1 : 0; } } }),
  ...
  killDraw: (name = 'Trophy') => ({ name, text: 'Whenever this Identity defeats an enemy Identity, draw a card.', hooks: { on(E, src, ev, d) { if (ev === 'defeated' && d.killer === src.unit && d.owner !== src.owner) E.draw(src.owner, 1); } } }),
  ...
  summonSap: (n, name = 'Forage') => ({ name, text: `When summoned: gain ${n} Sap.`, hooks: { on(E, src, ev, d) { if (ev === 'summoned' && d.unit === src.unit) E.gainSap(src.owner, n); } } }),
  mpBattery: (n, name = 'Deep Roots') => ({ name, text: `At the start of your turn, this Identity restores ${n} additional MP.`, hooks: { on(E, src, ev, d) { if (ev === 'turnStart' && d.p === src.owner) E.restoreMp(me(E, src), n); } } }),
  ...
  inspire: (name = 'Inspire') => ({ name, text: 'When summoned: other friendly Identities in this Lane gain +1 AP this turn.', hooks: { on(E, src, ev, d) { if (ev !== 'summoned' || d.unit !== src.unit) return; const m = me(E, src); for (const f of E.unitsInLane(laneU(m), m.owner)) if (f.iid !== m.iid) E.addApBonus(f, 1, name); } } }),
  ...
  equipDiscount: (name = 'Handy') => ({ name, text: 'Equipment attached to this Identity costs 1 less Sap.', hooks: { cardCost: (E, src, p, d, ctx) => (p === src.owner && d.type === 'Equipment' && ctx.target === src.unit ? -1 : 0) } }),
  consDiscount: (name = 'Home Cooking') => ({ name, text: 'Consumables you attach cost 1 less Sap. (Does not stack.)', hooks: { cardCost: (E, src, p, d) => (p === src.owner && d.type === 'Consumable' ? { v: -1, key: 'consDisc' } : 0) } }),
  killRefresh: (name = 'Trophy Hunt') => ({ name, text: 'Once per turn, when this Identity defeats an enemy Identity, it may attack again.', hooks: { on(E, src, ev, d) { if (ev === 'defeated' && d.killer === src.unit && d.owner !== src.owner) { const m = me(E, src); if (m && E.once(m, 'trophyHunt')) { m.extraAttacks++; E.emit({ t: 'status', target: m.iid, text: 'Attack again!', good: true }); } } } } }),
  ...
  collision: (n, name = 'Undertow') => ({ name, text: `Enemies that collide after being forcibly moved take ${n} additional damage.`, hooks: { collisionBonus: (E, src, u, byP) => (byP === src.owner && u.owner !== src.owner ? n : 0) } }),
};
...
function identityStats(cls, cost, rarity, mod = {}) {
  const B = C.ISB_BASE + C.ISB_PER_COST * cost + (C.RARITY_ISB_BONUS[rarity] || 0);
  const w = C.CLASS_WEIGHTS[cls];
  const s = {};
  for (const k of ['bp', 'sp', 'mp', 'ap', 'rp']) s[k] = Math.round(B * w[k] * (mod[k] || 1));
  s.bp = Math.max(1, s.bp);
  s.sp = Math.max(1, s.sp);
  s.mp = Math.max(1, s.mp);
  s.ap = Math.min(6, Math.max(1, s.ap));
  s.rp = Math.min(4, Math.max(1, s.rp));
...
const IDENTITIES = {
  ...
  Forager: [
    ...
    ['Tinkerer Beetle', 'Arthropod', 'Labor', 2, '🪲', T.equipDiscount('Handy Mandibles'), {}, 'Fixes anything. Eats the leftovers.'],
    ...
    ['Mira the Wandering Cook', 'Character', 'Monopoly', 3, '🥘', T.consDiscount('Home Cooking'), {}, '"Nobody fights on an empty stomach. Eat!"'],
    ...
    ['Newt Alchemist', 'Scales', 'Stagnation', 3, '⚗️', T.sapTap(1, 2, 'Transmute'), {}, 'Lead into gold? Pah. Mud into Sap.'],
...
// ===========================================================================
const inLane = (src, u) => laneU(u) === src.lane;
const ZONES = {
  Environmental: [
    ['Dewfall Glade', 'Feathered', 'Succulence', 1, '🌿', 'At the start of each player\'s turn, their Identities in this Lane restore 1 BP.', { on(E, src, ev, d) { if (ev === 'turnStart') for (const u of E.unitsInLane(src.lane, d.p)) E.heal(u, 1); } }],
    ['Misty Hollow', 'Ghoul', 'Obscurity', 1, '🌫️', 'Identities in this Lane have -1 RP (minimum 1).', { stat: (E, src, u, k) => (k === 'rp' && inLane(src, u) ? -1 : 0) }],
    ['Sunbeam Meadow', 'Pelted', 'Labor', 1, '🌻', 'At the end of an Identity\'s activation in this Lane, it restores 1 MP.', { on(E, src, ev, d) { if (ev === 'activationEnd') { const u = E.unit(d.unit); if (u && inLane(src, u)) E.restoreMp(u, 1); } } }],
    ['Spore Fog', 'Magically-Mutated', 'Venom', 1, '🍄', 'Abilities used by enemy Identities in this Lane cost 1 additional MP.', { abilityCost: (E, src, u) => (inLane(src, u) && u.owner !== src.owner ? 1 : 0) }],
    ['Moonlit Clearing', 'Character', 'Seclusion', 1, '🌙', 'Abilities used by your Identities in this Lane cost 1 less MP.', { abilityCost: (E, src, u) => (inLane(src, u) && u.owner === src.owner ? -1 : 0) }],
    ['Stinging Nettlefield', 'Arthropod', 'Erosion', 2, '🌾', 'At the start of your turn, each enemy Identity in this Lane takes 1 damage.', { on(E, src, ev, d) { if (ev === 'turnStart' && d.p === src.owner) for (const u of E.unitsInLane(src.lane, 1 - src.owner)) E.dealDamage(u.iid, 1, { kind: 'hazard', p: src.owner }); } }],
    ['Ancient Spring', 'Scales', 'Persistence', 2, '⛲', 'At the start of each player\'s turn, their Identities in this Lane restore 1 MP and 1 BP.', { on(E, src, ev, d) { if (ev === 'turnStart') for (const u of E.unitsInLane(src.lane, d.p)) { E.restoreMp(u, 1); E.heal(u, 1); } } }],
  ],
  Terrain: [
    ['Tangled Thicket', 'Pelted', 'Stagnation', 1, '🌳', 'Enemy Identities must spend 1 additional AP to enter this Lane.', { moveCost: (E, src, u, from, to) => (u.owner !== src.owner && laneX(to.x) === src.lane && laneX(from.x) !== src.lane ? 1 : 0) }],
    ['Mossy Path', 'Arthropod', 'Labor', 1, '🟩', 'The first tile an Identity moves each activation within this Lane costs no AP.', { firstStepFree: (E, src, u) => u.state !== 'done' && inLane(src, u) }],
    ['Muddy Bank', 'Scales', 'Erosion', 1, '🟫', 'Forced movement within this Lane moves an additional tile.', { forcedDelta: (E, src, u) => (inLane(src, u) ? 1 : 0) }],
    ['Hollow Log Run', 'Feathered', 'Obscurity', 1, '🪵', 'Identities in this Lane may move through occupied tiles within it.', { passThrough: (E, src, u, t) => inLane(src, u) && laneX(t.x) === src.lane }],
    ['Sheer Ravine', 'Knot-Bots', 'Hardpan', 1, '🏔️', 'Identities in this Lane cannot be forcibly moved.', { forcedImmune: (E, src, u) => inLane(src, u) }],
    ['Rolling Hills', 'Character', 'Friction', 2, '⛰️', 'Your Identities that begin their activation in this Lane gain +1 AP.', { on(E, src, ev, d) { if (ev === 'activationStart') { const u = E.unit(d.unit); if (u && u.owner === src.owner && inLane(src, u)) E.addApBonus(u, 1, 'Rolling Hills'); } } }],
    ['Briar Maze', 'Magically-Mutated', 'Seclusion', 2, '🌹', 'Enemy Identities must spend 1 additional AP to leave this Lane.', { moveCost: (E, src, u, from, to) => (u.owner !== src.owner && laneX(from.x) === src.lane && laneX(to.x) !== src.lane ? 1 : 0) }],
  ],
  Property: [
    ['Warren Commons', 'Pelted', 'Ancestry', 1, '🏡', 'Your Identities in this Lane gain +1 SP while adjacent to another friendly Identity.', { stat: (E, src, u, k) => (k === 'sp' && u.owner === src.owner && inLane(src, u) && E.friendlyAdjacent(u).length ? 1 : 0) }],
    ['Watchpost Grove', 'Feathered', 'Obscurity', 1, '🗼', 'Once per turn, when an enemy Identity enters this Lane, one of your Identities in this Lane may move 1 tile.', { on(E, src, ev, d) { if (ev !== 'enterLane' || d.lane !== src.lane) return; const u = E.unit(d.unit); if (!u || u.owner === src.owner) return; const f = E.unitsInLane(src.lane, src.owner)[0]; if (f && E.once(E.s.lanes[src.lane], 'watchpost')) E.grantFreeSteps(f, 1, 'Watchpost'); } }],
    ...
    ['Toll Bridge', 'Knot-Bots', 'Scarcity', 1, '🌉', 'When an enemy Identity enters this Lane, it loses 1 MP.', { on(E, src, ev, d) { if (ev === 'enterLane' && d.lane === src.lane) { const u = E.unit(d.unit); if (u && u.owner !== src.owner) E.loseMp(u, 1); } } }],
    ['Crowded Burrows', 'Arthropod', 'Monopoly', 1, '🕳️', 'Your Identities in this Lane take 1 less damage while adjacent to 2 or more friendly Identities.', { incoming(E, src, dmg) { const u = E.unit(dmg.target); if (u && u.owner === src.owner && inLane(src, u) && E.friendlyAdjacent(u).length >= 2) dmg.amount -= 1; } }],
    ['Rookery Heights', 'Feathered', 'Friction', 2, '🕊️', 'If 3 or more of your Identities occupy this Lane, they gain +1 AP.', { stat: (E, src, u, k) => (k === 'ap' && u.owner === src.owner && inLane(src, u) && E.unitsInLane(src.lane, src.owner).length >= 3 ? 1 : 0) }],
    ['Gathering Stump', 'Ghoul', 'Labor', 2, '🪑', 'At the start of your turn, if 2 or more of your Identities occupy this Lane, gain 1 Sap.', { on(E, src, ev, d) { if (ev === 'turnStart' && d.p === src.owner && E.unitsInLane(src.lane, src.owner).length >= 2) E.gainSap(src.owner, 1); } }],
  ],
  Domain: [
    ...
    ['Nursery Bed', 'Feathered', 'Ancestry', 1, '🥚', 'Identities summoned in this Lane gain +1 SP until the end of the turn.', { on(E, src, ev, d) { if (ev === 'summoned') { const u = E.unit(d.unit); if (u && u.owner === src.owner && inLane(src, u)) E.addStatus(u, eot('sp', 1, '+1 SP · Nursery')); } } }],
    ['Sap Wells', 'Scales', 'Succulence', 1, '🍯', 'Whenever you summon an Identity in this Lane, restore 2 BP to your Structure here.', { on(E, src, ev, d) { if (ev === 'summoned') { const u = E.unit(d.unit); const st = E.structInLane(src.lane); if (u && u.owner === src.owner && inLane(src, u) && st && st.owner === src.owner) E.healStruct(st, 2); } } }],
    ['Builder\'s Clearing', 'Character', 'Labor', 1, '🪓', 'Structures you build in this Lane cost 1 less Sap.', { cardCost: (E, src, p, d, ctx) => (p === src.owner && d.type === 'Structure' && ctx.target === src.lane ? -1 : 0) }],
    ['Old Growth', 'Magically-Mutated', 'Persistence', 2, '🌲', 'Your Structure in this Lane has +4 maximum BP.', { structBp: (E, src, st) => (st.lane === src.lane && st.owner === src.owner ? 4 : 0) }],
    ['Mycelial Web', 'Ghoul', 'Infrastructure', 2, '🕸️', 'Identities you summon in this Lane gain +1 AP on the turn they are summoned.', { on(E, src, ev, d) { if (ev === 'summoned') { const u = E.unit(d.unit); if (u && u.owner === src.owner && inLane(src, u)) E.addApBonus(u, 1, 'Mycelial Web'); } } }],
  ],
  Territory: [
    ['Contested Border', 'Pelted', 'Friction', 1, '🚧', 'Your Identities in this Lane gain +1 SP.', { stat: (E, src, u, k) => (k === 'sp' && u.owner === src.owner && inLane(src, u) ? 1 : 0) }],
    ['Snarlwood Frontier', 'Magically-Mutated', 'Stagnation', 1, '🌵', 'Enemy Identities invading this Lane have -1 AP.', { stat: (E, src, u, k) => (k === 'ap' && u.owner !== src.owner && inLane(src, u) ? -1 : 0) }],
    ...
    ['Battle-Scarred Field', 'Ghoul', 'Erosion', 1, '⚔️', 'Attacks made by Identities in this Lane gain +1 SP while it is contested.', { attackBonus: (E, src, u) => (inLane(src, u) && E.isContested(src.lane) ? 1 : 0) }],
    ['Watchtower Ridge', 'Feathered', 'Obscurity', 1, '🔭', 'Your Identities in this Lane gain +1 RP.', { stat: (E, src, u, k) => (k === 'rp' && u.owner === src.owner && inLane(src, u) ? 1 : 0) }],
    ['Bloodmoss Marches', 'Scales', 'Venom', 2, '🩸', 'Enemy Identities that end their activation in this Lane take 1 damage.', { on(E, src, ev, d) { if (ev === 'activationEnd') { const u = E.unit(d.unit); if (u && u.owner !== src.owner && inLane(src, u)) E.dealDamage(u.iid, 1, { kind: 'hazard', p: src.owner }); } } }],
    ['Ancestral Grounds', 'Arthropod', 'Ancestry', 2, '🏯', 'At the start of your turn, gain 1 additional Renown from this Lane.', { on(E, src, ev, d) { if (ev === 'turnStart' && d.p === src.owner) E.gainRenown(src.owner, 1, 'Ancestral Grounds'); } }],
  ],
};
...
const STRUCTS = {
  Shelter: [
    ...
    ['Lantern Cottage', 'Character', 'Seclusion', 3, 10, 2, '🏮', 'At the start of your turn, restore 1 MP to each friendly Identity within 2 tiles.', { on(E, src, ev, d) { if (ev === 'turnStart' && d.p === src.owner) for (const u of E.unitsOf(src.owner)) if (nearSt(E, src, u)) E.restoreMp(u, 1); } }],
  ...
  Bastion: [
    ...
    ['Watch Keep', 'Feathered', 'Stagnation', 3, 12, 1, '🗼', 'Enemy Identities within 2 tiles of this Structure have -1 AP.', { stat: (E, src, u, k) => (k === 'ap' && u.owner !== src.owner && nearSt(E, src, u) ? { v: -1, key: 'watchKeep' } : 0) }],
    ['Spiked Barricade', 'Pelted', 'Erosion', 3, 13, 1, '🪤', 'Enemy Identities that end their movement within 1 tile of this Structure take 1 damage.', { on(E, src, ev, d) { if (ev === 'moved') { const u = E.unit(d.unit); if (u && u.owner !== src.owner && nearSt(E, src, u, 1)) E.dealDamage(u.iid, 1, { kind: 'hazard', p: src.owner }); } } }],
  ...
  Den: [
    ...
    ['Hive Nest', 'Arthropod', 'Labor', 2, 6, 3, '🍯', 'Identities summoned through this Structure cost 1 less Sap (minimum 1).', { cardCost: (E, src, p, d, ctx) => (p === src.owner && d.type === 'Identity' && ctx.struct === src.struct ? -1 : 0) }],
    ...
    ['Spawning Pool', 'Ghoul', 'Persistence', 3, 7, 4, '💧', 'When a friendly Identity summoned here is defeated, gain 1 Sap.', { on(E, src, ev, d) { if (ev === 'defeated' && d.via === src.struct) E.gainSap(src.owner, 1); } }],
    ['Roost', 'Feathered', 'Obscurity', 3, 7, 3, '🐓', 'You may summon Identities through this Structure onto any empty tile within 2 tiles of it.', { summonRange: (E, src, st) => (st.iid === src.struct ? 1 : 0) }],
  ...
  Landmark: [
    ['Old Totem', 'Character', 'Ancestry', 2, 9, 2, '🗿', 'Your Identities in this Lane gain +1 RP.', { stat: (E, src, u, k) => (k === 'rp' && u.owner === src.owner && laneU(u) === src.lane ? 1 : 0) }],
    ['Weeping Willow', 'Ghoul', 'Venom', 2, 9, 2, '🌳', 'Enemy Identities in this Lane have -1 SP.', { stat: (E, src, u, k) => (k === 'sp' && u.owner !== src.owner && laneU(u) === src.lane ? -1 : 0) }],
    ['Great Stump', 'Pelted', 'Friction', 3, 10, 2, '🪵', 'Your Identities in this Lane gain +1 SP.', { stat: (E, src, u, k) => (k === 'sp' && u.owner === src.owner && laneU(u) === src.lane ? 1 : 0) }],
    ['Beacon Pine', 'Feathered', 'Labor', 3, 10, 2, '🎄', 'Your Identities that begin their activation in this Lane gain +1 AP.', { on(E, src, ev, d) { if (ev === 'activationStart') { const u = E.unit(d.unit); if (u && u.owner === src.owner && laneU(u) === src.lane) E.addApBonus(u, 1, 'Beacon Pine'); } } }],
    ['Standing Stones', 'Magically-Mutated', 'Infrastructure', 3, 11, 2, '🪨', 'Abilities of your Identities in this Lane cost 1 less MP.', { abilityCost: (E, src, u) => (u.owner === src.owner && laneU(u) === src.lane ? -1 : 0) }],
    ['Moon Pool', 'Scales', 'Succulence', 3, 10, 2, '🌕', 'At the start of your turn, your Identities in this Lane restore 1 MP and 1 BP.', { on(E, src, ev, d) { if (ev === 'turnStart' && d.p === src.owner) for (const u of E.unitsInLane(src.lane, src.owner)) { E.restoreMp(u, 1); E.heal(u, 1); } } }],
    ['Knotwood Clocktower', 'Knot-Bots', 'Monopoly', 4, 13, 2, '🕰️', 'At the start of your turn, if you control 3 or more Lanes, gain 1 additional Renown.', { on(E, src, ev, d) { if (ev === 'turnStart' && d.p === src.owner && E.s.lanes.filter((l) => l.ctrl === src.owner).length >= 3) E.gainRenown(src.owner, 1, 'Clocktower'); } }],
  ],
  Facility: [
    ['Tinker\'s Workshop', 'Knot-Bots', 'Labor', 2, 9, 2, '🔧', 'Equipment you attach costs 1 less Sap. (Does not stack.)', { cardCost: (E, src, p, d) => (p === src.owner && d.type === 'Equipment' ? { v: -1, key: 'workshop' } : 0) }],
    ['Apothecary', 'Scales', 'Succulence', 2, 9, 2, '⚗️', 'Consumables you attach cost 1 less Sap. (Does not stack.)', { cardCost: (E, src, p, d) => (p === src.owner && d.type === 'Consumable' ? { v: -1, key: 'consDisc' } : 0) }],
    ['Sap Press', 'Arthropod', 'Scarcity', 3, 9, 2, '🏭', 'At the start of your turn, gain 1 Sap.', { on(E, src, ev, d) { if (ev === 'turnStart' && d.p === src.owner) E.gainSap(src.owner, 1); } }],
    ...
    ['Archive Hollow', 'Character', 'Obscurity', 3, 10, 2, '📚', 'At the start of your turn, if you have 3 or fewer cards in hand, draw a card.', { on(E, src, ev, d) { if (ev === 'turnStart' && d.p === src.owner && E.P(src.owner).hand.length <= 3) E.draw(src.owner, 1); } }],
    ...
    ['Knotwood Laboratory', 'Knot-Bots', 'Infrastructure', 4, 12, 3, '🧪', 'At the start of your turn, gain 1 Sap and restore 1 MP to each of your Identities.', { on(E, src, ev, d) { if (ev === 'turnStart' && d.p === src.owner) { E.gainSap(src.owner, 1); for (const u of E.unitsOf(src.owner)) E.restoreMp(u, 1); } } }],
  ],
};
...
const EQUIP = {
  Tool: [
    ...
    ['Lantern of Dusk', 'Ghoul', 'Seclusion', 2, '🏮', 'At the start of your turn, the equipped Identity restores 1 additional MP.', { on(E, src, ev, d) { if (ev === 'turnStart' && d.p === src.owner) E.restoreMp(E.unit(src.unit), 1); } }],
  ...
  Weapon: [
    ...
    ['Venom Stinger', 'Arthropod', 'Venom', 2, '🦂', '+1 SP. Enemies damaged by the equipped Identity\'s attacks lose 1 AP until the end of their next activation.', { stat: eqStat({ sp: 1 }), on(E, src, ev, d) { if (ev === 'damaged' && d.srcUnit === src.unit && d.kind === 'attack') { const t = E.unit(d.target); if (t && t.owner !== src.owner) actDebuff(E, t, 'ap', -1, '-1 AP · Venom'); } } }],
  ...
  Trap: [
    ['Snare Wire', 'Pelted', 'Stagnation', 1, '🪢', 'Once per turn, when an enemy Identity moves adjacent to the equipped Identity, it loses its remaining AP.', { on(E, src, ev, d) { if (ev !== 'step') return; const m = E.unit(src.unit); const e = E.unit(d.unit); if (m && e && e.owner !== m.owner && ch(e, m) === 1 && e.state === 'active' && E.once(m, 'snare')) { e.apSpent += E.apAvail(e); e.freeSteps = 0; e.moveStopped = true; E.emit({ t: 'status', target: e.iid, text: 'Snared!', good: false }); } } }],
    ['Pitfall Stakes', 'Knot-Bots', 'Erosion', 1, '🪤', 'Once per turn, an enemy Identity that ends its movement adjacent to the equipped Identity takes 2 damage.', { on(E, src, ev, d) { if (ev !== 'moved') return; const m = E.unit(src.unit); const e = E.unit(d.unit); if (m && e && e.owner !== m.owner && ch(e, m) === 1 && E.once(m, 'pitfall')) E.dealDamage(e.iid, 2, { kind: 'effect', unit: m.iid, p: m.owner }); } }],
    ...
    ['Tripwire Bell', 'Feathered', 'Obscurity', 1, '🔔', 'Once per turn, when an enemy Identity moves within 2 tiles of the equipped Identity, it may move 1 tile.', { on(E, src, ev, d) { if (ev !== 'moved') return; const m = E.unit(src.unit); const e = E.unit(d.unit); if (m && e && e.owner !== m.owner && ch(e, m) <= 2 && E.once(m, 'tripwire')) E.grantFreeSteps(m, 1, 'Tripwire'); } }],
...
const CONS = {
  Trinket: [
    ...
    ['Mirror Shard', 'Magically-Mutated', 'Seclusion', 1, '🪞', 'Use: swap places with an adjacent Identity.', { targets: [{ type: 'unit', side: 'any', range: 1, notSelf: true, label: 'Swap with whom?' }], resolve: (E, c) => { const t = E.unit(c.targets[0]); if (t.owner !== c.p && E.hookAny('forcedImmune', t, c.p)) return; E.swap(E.unit(c.unit), t); } }],
  ...
  Provision: [
    ...
    ['Feast Basket', 'Character', 'Ancestry', 2, '🧺', 'Use: restore 2 BP to each friendly Identity in the equipped Identity\'s Lane, then draw a card.', selfUse((E, c) => { const m = E.unit(c.unit); for (const u of E.unitsInLane(laneU(m), c.p)) E.heal(u, 2, { kind: 'consumable' }); E.draw(c.p, 1); }), true],
  ],
  Catalyst: [
    ['Adrenal Sap', 'Magically-Mutated', 'Friction', 1, '💉', 'Use: the equipped Identity may attack again this turn.', selfUse((E, c) => { const u = E.unit(c.unit); u.extraAttacks++; E.emit({ t: 'status', target: u.iid, text: 'Attack again!', good: true }); }, (E, c) => E.unit(c.unit).attacks > 0)],
    ['Surge Spore', 'Arthropod', 'Scarcity', 1, '⚡', 'Use: +3 AP this turn.', selfUse((E, c) => E.addApBonus(E.unit(c.unit), 3, 'Surge Spore'))],
    ['Overcharge Cell', 'Knot-Bots', 'Labor', 1, '🔋', 'Use: gain 2 Sap. If the equipped Identity is a Knot-Bot, gain 3 instead.', selfUse((E, c) => E.gainSap(c.p, E.faction(E.unit(c.unit)) === 'Knot-Bots' ? 3 : 2))],
...
const ACTIONS = {
  Interaction: [
    ...
    ['Sap Siphon', 'Magically-Mutated', 'Scarcity', 1, '🩸', 'Target enemy Identity loses 2 MP. Gain 1 Sap.', { targets: [enemy('Siphon which enemy?')], resolve: (E, c) => { E.loseMp(E.unit(c.targets[0]), 2); E.gainSap(c.p, 1); } }],
  ...
  Command: [
    ...
    ['Muster', 'Arthropod', 'Infrastructure', 1, '📣', 'The next Identity you summon this turn costs 2 less Sap (minimum 1). Draw a card.', { targets: [], resolve: (E, c) => { E.addMod('KF-MUSTER', c.p, { at: 'eot' }); E.draw(c.p, 1); } }],
    ...
    ['Regroup', 'Ghoul', 'Persistence', 3, '🔁', 'Target friendly Identity that has finished its activation may activate again this turn.', { targets: [friendly('Regroup which Identity?', { filter: (E, u) => u.state === 'done' })], resolve: (E, c) => E.refreshUnit(E.unit(c.targets[0])) }],
  ],
  Maneuver: [
    ...
    ['Flank', 'Ghoul', 'Predation', 2, '🌀', 'Move target friendly Identity to an empty tile adjacent to an enemy Identity within 4 tiles of it.', { targets: [friendly('Who flanks?'), { type: 'tile', label: 'Flank to where?', filter: (E, t, ctx) => { const u = E.unit(ctx.targets[0]); return u && ch(u, t) <= 4 && E.allUnits().some((e) => e.owner !== ctx.p && ch(e, t) === 1); } }], resolve: (E, c) => E.teleport(E.unit(c.targets[0]), c.targets[1]) }],
    ['Burrow Tunnel', 'Pelted', 'Infrastructure', 2, '🕳️', 'Move target friendly Identity to any empty tile in a Lane you control.', { targets: [friendly('Who tunnels?'), { type: 'tile', label: 'Emerge where?', filter: (E, t, ctx) => E.laneCtrl(laneX(t.x)) === ctx.p }], resolve: (E, c) => E.teleport(E.unit(c.targets[0]), c.targets[1]) }],
    ['Hit and Run', 'Character', 'Friction', 2, '🏃', 'Target friendly Identity that has attacked this turn may attack again and gains +2 AP.', { targets: [friendly('Who strikes again?', { filter: (E, u) => u.attacks > 0 })], resolve: (E, c) => { const u = E.unit(c.targets[0]); if (u.state === 'done') E.refreshUnit(u); u.extraAttacks++; E.addApBonus(u, 2, 'Hit and Run'); } }],
  ],
  Technique: [
    ...
    ['Double Tap', 'Character', 'Predation', 2, '✌️', 'Target friendly Identity that has attacked this turn may attack again.', { targets: [friendly('Who attacks again?', { filter: (E, u) => u.attacks > 0 })], resolve: (E, c) => { const u = E.unit(c.targets[0]); if (u.state === 'done') E.refreshUnit(u); u.extraAttacks++; } }],
...
const EVENTS = {
  Object: [
    ...
    ['Sap Geyser', 'Knot-Bots', 'Succulence', 2, '⛲', 'Instant', 'Gain 3 Sap.', { targets: [], resolve: (E, c) => E.gainSap(c.p, 3) }],
  ],
  Weather: [
    ...
    ['Gale', 'Feathered', 'Hardpan', 2, '🌬️', 'Instant', 'Push each enemy Identity 1 tile toward its controller\'s side of the battlefield.', { targets: [], resolve: (E, c) => { const dir = c.p === 0 ? 1 : -1; const es = E.allUnits().filter((u) => u.owner !== c.p).sort((a, b) => (b.y - a.y) * dir); for (const u of es) E.forceMove(u, { dx: 0, dy: dir }, 1, c.p); } }],
    ...
    ['Thunderstorm', 'Magically-Mutated', 'Erosion', 3, '⛈️', 'Instant', 'Deal 2 damage to up to 3 random enemy Identities.', { targets: [], condition: (E, c) => E.unitsOf(1 - c.p).length > 0, resolve: (E, c) => { const es = E.shuffle(E.unitsOf(1 - c.p).map((u) => u.iid)).slice(0, 3); for (const id of es) E.dealDamage(id, 2, { kind: 'effect', p: c.p }); } }],
    ['Heatwave', 'Pelted', 'Stagnation', 3, '☀️', 'Until end of their next turn', 'Enemy Identities have -1 AP until the end of your opponent\'s next turn.', { targets: [], resolve: (E, c) => E.addMod(c.cardId, c.p, { at: 'endNext', p: 1 - c.p }) }, { stat: (E, src, u, k) => (k === 'ap' && u.owner !== src.owner ? -1 : 0) }],
  ],
  'Space-Time': [
    ...
    ['Warp Rift', 'Magically-Mutated', 'Obscurity', 2, '🌀', 'Instant', 'Swap the positions of two target Identities.', { targets: [{ type: 'unit', side: 'any', label: 'First Identity' }, { type: 'unit', side: 'any', label: 'Second Identity', filter: (E, u, ctx) => u.iid !== ctx.targets[0] }], resolve: (E, c) => { const a = E.unit(c.targets[0]); const b = E.unit(c.targets[1]); for (const u of [a, b]) if (u.owner !== c.p && E.hookAny('forcedImmune', u, c.p)) return; E.swap(a, b); } }],
    ['Stasis Field', 'Ghoul', 'Stagnation', 2, '🧊', 'Until end of their next turn', 'Target enemy Identity cannot move or attack until the end of its controller\'s next turn.', { targets: [enemy('Freeze which enemy?')], resolve: (E, c) => E.addStatus(E.unit(c.targets[0]), { kind: 'stasis', at: 'endNext', p: 1 - c.p, label: 'Stasis', good: false }) }],
    ['Echo of Tomorrow', 'Character', 'Persistence', 2, '🔮', 'Your next 2 turns', 'At the start of your next 2 turns, draw a card.', { targets: [], resolve: (E, c) => E.addMod(c.cardId, c.p, { at: 'none' }, { left: 2 }) }, { on(E, src, ev, d) { if (ev === 'turnStart' && d.p === src.owner) { const m = E.s.mods.find((x) => x.id === src.mod); if (m && m.data.left > 0) { m.data.left--; E.draw(src.owner, 1); if (m.data.left <= 0) { E.s.mods = E.s.mods.filter((x) => x !== m); E.dirty(); } } } } }],
  ...
  Customs: [
    ['Harvest Festival', 'Pelted', 'Succulence', 2, '🎑', 'Instant', 'Restore 2 BP to each of your Identities. Gain 1 Sap.', { targets: [], resolve: (E, c) => { for (const u of E.unitsOf(c.p)) E.heal(u, 2); E.gainSap(c.p, 1); } }],
    ['Truce Day', 'Character', 'Hardpan', 2, '🕊️', 'Until end of their next turn', 'Structures cannot be damaged until the end of your opponent\'s next turn.', { targets: [], resolve: (E, c) => E.addMod(c.cardId, c.p, { at: 'endNext', p: 1 - c.p }) }, { incoming(E, src, dmg) { if (dmg.isStruct) dmg.amount = 0; } }],
    ...
    ['Tribute', 'Ghoul', 'Scarcity', 2, '💸', 'Instant', 'Your opponent loses 2 Sap and discards a random card.', { targets: [], resolve: (E, c) => { const o = 1 - c.p; E.gainSap(o, -Math.min(2, E.P(o).sap)); const h = E.P(o).hand; if (h.length) E.discardFromHand(o, h[E.randInt(h.length)]); } }],
...
function build() {
  for (let c = 0; c < 5; c++) {
    ...
    STRUCTS[scls].forEach((row, i) => {
      const [name, faction, archetype, cost, bp, housing, emoji, text, hooks] = row;
      register({ type: 'Structure', name, cls: scls, classIndex: c, faction, archetype, rarity: RARITY_PLAN.Structure[i], cost, bp, housing, emoji, text, hooks: hooks || null });
    });
...
REGISTRY.set('KF-MUSTER', { id: 'KF-MUSTER', token: true, type: 'Event', name: 'Muster', modHooks: {
  cardCost(E, src, p, d) { return p === src.owner && d.type === 'Identity' ? -2 : 0; },
  on(E, src, ev, d) { if (ev === 'summoned' && !d.token) { const u = E.unit(d.unit); if (u && u.owner === src.owner) { E.s.mods = E.s.mods.filter((m) => m.id !== src.mod); E.dirty(); } } },
} });
